...
- **远程终端兼容** — 使用 `TERM=xterm-256color`，兼容常见 SSH 服务器的 terminfo 和 Ubuntu 默认 Bash 彩色提示符检测
...
|---------------|----------------|----------------|----------|-------------|
| `<A-Left>` | `Alt+Left` | `Option+Left` | `ESC [ 1 ; 3 D` | 位置历史后退 |
| `<A-Right>` | `Alt+Right` | `Option+Right` | `ESC [ 1 ; 3 C` | 位置历史前进 |
