...
starship 通过 `integrations.zsh` 的 `eval "$(starship init zsh)"` 接入，配置部署由 `install.sh` 的 `shared_configs` 完成。当前提示符采用统一透明底色的 Catppuccin 极简 Powerline 风格：左侧显示系统、目录、Git、常用开发环境（C/Rust/Python/Node/Bun/Docker）和后台作业数，使用前景色与 Nerd Font 图标区分信息类型；右侧显示时间与超过 2 秒的命令耗时；Conda `base` 环境默认隐藏，仅显示具体环境。
