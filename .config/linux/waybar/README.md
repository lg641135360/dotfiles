...
- **CPU/内存**：使用 waybar 内置 `cpu`、`memory` 模块（kernel 级采样，每个模块独立持有基线，不受多 bar 并发或 waybar 重载影响，根除此前 custom 脚本共享 state 文件导致的偶发 0% 问题）。`format` 显示 `{usage}%`/`{percentage}%`，`tooltip-format` 显示使用率与负载/已用容量；`states` 驱动 warning/critical 配色（CPU 70/90，内存 80/95），单击拉起 `foot -- htop` 按对应字段排序；`interval 5s` 控制刷新频率
