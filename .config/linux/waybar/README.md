...
左: [工作区 │ 窗口标题]
中: [时钟]
右: [网络 CPU 内存 音量 空闲抑制 隐私状态 系统托盘]

aarch64（config.aarch64，含背光与电池）
左: [工作区 │ 窗口标题]
中: [时钟]
右: [网络 CPU 内存 背光 音量 电池 空闲抑制 隐私状态 系统托盘]
```
...
- **空闲抑制**：`idle_inhibitor` 模块单击切换 swayidle 自动锁屏/关屏（看长视频、演示等场景）；激活时眼睛图标变 peach 色醒目提示，tooltip 显示当前状态（"自动锁屏已停用/运行中"）
