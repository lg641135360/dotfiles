...
- **CPU/内存**：用 `custom/cpu`、`custom/memory` 替换 waybar 内置模块，后端脚本 `~/.config/scripts/waybar-system-tooltip` 子命令 `cpu`/`mem` 输出 JSON（text+tooltip+percentage+class）；栏内显示利用率，hover tooltip 显示使用率摘要与 top 5 进程（对齐 AwesomeWM `widgets/system.lua` 风格）；CPU 使用率基于 `/proc/stat` 差值（状态文件 `$XDG_STATE_HOME/dotfiles/waybar-cpu`），内存基于 `/proc/meminfo` MemAvailable；states 配色通过 JSON `class` 字段驱动（CPU warning 70%/critical 90%，内存 warning 80%/critical 95%）；单击拉起 `kitty -- htop` 按对应字段排序。waybar `return-type=json` 模式下 `tooltip-exec` 会被忽略（tooltip 必须由 JSON 字段提供），故每次 `exec` 都计算 top 进程，`interval 5s` 控制常态 CPU 开销在 2-3%
