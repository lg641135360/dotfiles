...
- **CPU/内存**：使用 custom/cpu、custom/memory 模块（waybar 内置 cpu/memory 模块的 `tooltip-format` 占位符无法注入动态进程列表），后端 `~/.config/scripts/waybar-system-tooltip` 子命令 cpu/mem 输出 JSON（text+tooltip+percentage+class），栏内显示利用率，hover tooltip 显示使用率摘要 + top 5 进程（对齐 AwesomeWM `widgets/system.lua` 风格：首行模块名标题 + 摘要 + Top 进程列表，进程行 `pid  comm  value` 两空格分隔）；CPU 使用率基于 `/proc/stat` 单次调用内自差分（间隔 1s 采样两次，规避 state 文件在多 bar 并发 / waybar 重载时被覆盖导致偶发 0% 的问题），**top 进程同样基于 `/proc/<pid>/stat` 两次采样 utime+stime 差值算瞬时 %CPU**（与栏内 1s 平均使用率口径一致，不依赖 `ps %CPU` 的生命周期平均——长跑进程如 chrome 不会被 elapsed 稀释偏低、新进程不会偏高），显示一位小数（1s 窗口下 <1% 的瞬时值常见，整数截断会把低占用进程显示成 0%），复用同一 sleep 1 窗口不增加阻塞；内存基于 `/proc/meminfo` MemAvailable，top 内存进程用 `ps --sort=-rss` RSS 瞬时值（已准确无需差分）；states 配色通过 JSON `class` 字段驱动（CPU warning 70%/critical 90%，内存 warning 85%/critical 95%，内存阈值比 CPU 高以避免缓存常态占用高导致误报）；waybar `return-type=json` 模式 `tooltip-exec` 被忽略，tooltip 必须由 JSON 字段提供，故每次 exec 都计算 top 进程，`interval 5s` 控制常态 CPU 开销 2-3%；单击拉起 `foot -- htop` 按对应字段排序
