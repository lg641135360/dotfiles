...
- **状态项交互**：NET/CPU/MEM 不绑定点击动作，只在鼠标悬浮时显示内置 detail；NET detail 展示网卡接口名和带 `/s` 单位的上下行速率，CPU/MEM detail 展示使用率、load average 和 top 进程；VOL 保留左键静音和滚轮调音量，右键 VOL 会打开 `pavucontrol`。
