...
- **状态项交互**：NET/CPU/MEM 不绑定点击动作，只在鼠标悬浮时显示内置 detail；NET detail 展示网卡接口名和带 `/s` 单位的上下行速率，找不到匹配接口时主栏显示 `NET:N/A` 且 hover 显示 offline；CPU/MEM detail 展示使用率、load average 和 top 进程，并使用 5 秒后台缓存，hover 时不临时执行 `ps`；VOL 保留左键静音和滚轮调音量，右键 VOL 会打开 `pavucontrol`。
