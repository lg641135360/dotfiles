...
```
主屏: ┌─[标签]─[布局]─[锁屏]─│─────────[任务列表]─────────│[NET│CPU│MEM│BAT│VOL]─[托盘]─[时钟]─┐
副屏: ┌─[标签]─[布局]─────────[任务列表]─────────│[时钟]─┐
```
...
- **屏幕拓扑刷新**：外接屏热插拔、`xrandr` 改变几何或主屏切换后，会延迟重建各屏 wibar 内容，重新判断主屏状态区和 full/compact 模式；tag 和 prompt 继续复用，tasklist 只有在标题宽度规格变化时才重建。主屏右侧的 sysinfo / VOL / systray 也会优先复用已有 widget，只有规格从 full/compact 切换时才重建，避免重复后台轮询。
