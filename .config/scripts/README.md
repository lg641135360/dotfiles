...
| `trae-cn-wayland` | Trae CN (Electron) Wayland 启动器（Wayland 会话加 ozone-wayland + Wayland IME，禁用 Crashpad 崩溃报告以消除 aarch64 内核缺 `CONFIG_CHECKPOINT_RESTORE` 导致的 `prctl: Invalid argument (22)` 告警；X11 原样透传） |
