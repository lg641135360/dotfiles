...
## 窗口规则 / 视觉
- niri 钉钉主窗口由 `window-rule` 管理列宽与不透明度：匹配 `com.alibabainc.dingtalk`，默认 2/3 列宽并覆盖为 1.0 不透明，避免 Qt/CEF 经 XWayland 在 mtgpu 上叠透明出黑块；不强制浮动、不强制聚焦、不固定输出，会议窗口和对话框仍由应用自身管理。不匹配 `tblive`。2026-08-29：Flatpak 版钉钉（`com.dingtalk.DingTalk`，1.3G 冗余）已卸载，fuzzel 双入口收敛；`dingtalk://` 与 `dingtalk_std_ind://` handler 统一指向 deb 包装条目 `com.alibabainc.dingtalk.desktop`（dingtalk-wayland），钉钉只保留 deb 版单一路径。
