...
- 剪贴板管理（2026-08-28）：`wl-clip-persist` 持久化 + `cliphist` 历史检索，入口统一为 `clipboard-wayland`（`start` 前台 exec `wl-paste --watch wl-clip-persist --clipboard regular`；`history` 用 `cliphist list | fuzzel --dmenu --prompt "剪贴板 >"` → `cliphist decode | wl-copy`）。`wayland-autostart` 用 `run_once_logged wl-clip-persist 'wl-paste.*wl-clip-persist' <clipboard-wayland> start` 拉起守护；niri 用 `Mod+V`（V=粘贴直觉，当时空余）调 `clipboard-wayland history`，`Mod+Shift+V` 已固定给「切换浮动/平铺焦点」不得占用。`wl-clip-persist` 不在 Ubuntu 24.04 apt 源，走 Nix（nix-profile）；`cliphist` 用 apt（0.4.0）。`clipboard-wayland` 顶部对 Nix profile PATH 做补丁（niri environment{} 固定环境不含 `~/.nix-profile/bin`）。fuzzel 1.9.2 勿引入 1.11+ 专属选项（如 placeholder）。
