...
- 安装器在检测到 Zsh 时，应确保 `~/.zshenv` 逐行幂等包含两条：`export ZDOTDIR=$HOME/.config/zsh` 和 `skip_global_compinit=1`。后者必须在 `~/.zshenv` 而非仅 `$ZDOTDIR/.zshenv`：ZDOTDIR 未预置时 zsh 只读 `~/.zshenv`，随后 `/etc/zsh/zshrc` 就检查该开关（2026-09-13 回归实证：缺它时 niri spawn 的终端每次交互启动跑 Ubuntu 全局 compinit，4.8s vs 修复后 0.28s）。
...
- codex 同样不走 brew（2026-09-02）：与 claude-code 一致改用 `npm i -g @openai/codex` 全局安装（`bin/codex.js` 为 Node 脚本，依赖 node 运行时），升级用 `npm update -g @openai/codex`；brew 版与 npm 版二选一勿共存（PATH 遮蔽）。
- 对通过 `npm install -g` 安装到用户级 `/home/rikoo/.npm-global` 前缀的 CLI，在共享 zsh PATH 中追加 `$HOME/.npm-global/bin`。
- Bun 官方安装器的 `$HOME/.bun/bin` 同样在 Linux 共享 zsh PATH 中追加（`bun install -g` 的 CLI 随之可用，如 omp）。
- Starship 提示符关闭 Python 版本模块（`[python] disabled = true`）。用户不以 Python 为主力，且目录里有 `.py` 就会显示解释器版本，信息噪音大于收益。
