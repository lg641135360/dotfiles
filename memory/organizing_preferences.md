...
- 对 Awesome 顶栏状态项交互，优先把详情放到低干扰点击入口：NET 左键打开网络状态终端面板，CPU/MEM 左键打开 `btop`/`htop`/`top` 系统监控，VOL 保留左键静音和滚轮调音量，右键才打开 `pavucontrol`。
- 对 Awesome 顶栏 CPU/MEM 系统监控入口，优先复用/聚焦已有监控终端窗口；不要每次点击都新开一个 `htop`/`btop`/`top` 实例。
