...
test_clipboard_wayland_persists_and_queries_history() {
    ...
    assert_contains 'xclip -selection clipboard -t TARGETS -o' "$CLIPBOARD_SCRIPT"
    ...
    assert_contains 'cat "$snap" | wl-copy -t "$x11_img"' "$CLIPBOARD_SCRIPT"
    assert_contains 'cat "$snap" | xclip -selection clipboard -t "$wl_img" -i' "$CLIPBOARD_SCRIPT"
    assert_contains 'cat "$snap" | wl-copy' "$CLIPBOARD_SCRIPT"
    assert_contains 'cat "$snap" | xclip -selection clipboard -i' "$CLIPBOARD_SCRIPT"
