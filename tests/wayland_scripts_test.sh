...
test_wayland_screenshot_uses_selection_and_annotation() {
    ...
    assert_contains "--copy-command 'wl-copy -t image/png'" "$SCREENSHOT_SCRIPT"
...
EOF
    ...
    assert_contains 'wl-copy -t image/png' "$satty_args"
...
test_clipboard_wayland_persists_and_queries_history() {
    ...
    assert_contains 'wl-clip-persist --clipboard regular --ignore-event-on-error &' "$CLIPBOARD_SCRIPT"
    # 断言锁定实际调用行而非整个文件：脚本注释里会提到这两个不存在的参数。
    assert_not_contains 'wl-clip-persist --clipboard regular --read-timeout' "$CLIPBOARD_SCRIPT"
    assert_not_contains 'wl-clip-persist --clipboard regular --restore-clipboard-on-error' "$CLIPBOARD_SCRIPT"
    ...
    assert_contains 'kill -0 "$bridge_pid"' "$CLIPBOARD_SCRIPT"
    assert_not_contains 'wl-paste --watch wl-clip-persist' "$CLIPBOARD_SCRIPT"
    # X11 ↔ Wayland 轮询桥：仅在有 xclip + DISPLAY 时启动；双向同步文本与图片
    assert_contains 'command -v xclip' "$CLIPBOARD_SCRIPT"
    assert_contains 'start_x11_bridge &' "$CLIPBOARD_SCRIPT"
    # xclip 在 selection owner 失联时会无限阻塞，所有读写必须套 timeout
    assert_contains 'command -v timeout' "$CLIPBOARD_SCRIPT"
    assert_contains 'timeout --foreground 2 "$@"' "$CLIPBOARD_SCRIPT"
    assert_contains 'clip_run xclip -selection clipboard -t TARGETS -o' "$CLIPBOARD_SCRIPT"
    assert_contains "grep -oE 'image/(png|jpeg|gif)'" "$CLIPBOARD_SCRIPT"
    # 快照单次读取：源内容每轮只读 1 次落到临时文件，判空/哈希/推送复用同一份
    assert_contains '> "$snap"' "$CLIPBOARD_SCRIPT"
    assert_contains '[ -s "$snap" ]' "$CLIPBOARD_SCRIPT"
    assert_contains 'sha256sum < "$snap"' "$CLIPBOARD_SCRIPT"
    assert_contains 'clip_run wl-copy -t "$x11_img" < "$snap"' "$CLIPBOARD_SCRIPT"
    assert_contains 'clip_run xclip -selection clipboard -t "$wl_img" -i < "$snap"' "$CLIPBOARD_SCRIPT"
    assert_contains 'clip_run wl-copy < "$snap"' "$CLIPBOARD_SCRIPT"
    assert_contains 'clip_run xclip -selection clipboard -i < "$snap"' "$CLIPBOARD_SCRIPT"
