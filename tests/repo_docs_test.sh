...
assert_not_contains 'tools/' "$ROOT_README"
assert_file_not_exists "$REPO_ROOT/tools"
...
assert_file_exists "$REPO_ROOT/.config/linux/swaylock/README.md"
...
agents_index=$(grep -oE 'awesome\.md|nvim\.md|tmux\.md|rofi\.md|alacritty\.md|desktop\.md|niri\.md|waybar\.md|git\.md|codex\.md|dingtalk\.md|foot\.md|herdr\.md' "$AGENTS_DOC" | sort -u)
...
org_index=$(grep -oE 'awesome\.md|nvim\.md|tmux\.md|rofi\.md|alacritty\.md|desktop\.md|niri\.md|waybar\.md|git\.md|codex\.md|dingtalk\.md|foot\.md|herdr\.md' "$REPO_ROOT/memory/organizing_preferences.md" | sort -u)
