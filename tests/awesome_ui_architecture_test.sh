...
test_wibar_owns_bar_widget_creation() {
    ...
    assert_not_contains 'border_color = ctpp.surface1,' "$STATUS_AREA_FILE"
    assert_not_contains 'bg = ctpp.mantle,' "$STATUS_AREA_FILE"
    assert_not_contains 'table.insert(right_widgets, create_separator(ctpp))' "$STATUS_AREA_FILE"
...
test_wibar_refreshes_after_screen_topology_changes() {
    ...
    assert_contains 'local primary = awful.screen.focused()' "$WIBAR_FILE"
    assert_contains 'for _, c in ipairs(client.get(s)) do' "$WIBAR_FILE"
    assert_contains 'c:move_to_tag(target_tag)' "$WIBAR_FILE"
