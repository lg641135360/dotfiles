...
- 目的：按用户要求将本轮 dotfiles 仓库维护优化发布到远程 GitHub。
- 已做：发布前确认本地 `HEAD` 与 `origin/main` 同步；执行 shell 语法检查、`tests/run.sh full` 和 `git diff --check` 均通过；随后按 Lore 协议提交当前 README、X11 README 命名、Git memory、测试 helper、统一测试入口与相关测试改动为 `dc91f96`（`Keep dotfiles maintenance evidence consistent`），并推送到 `git@github.com:lg641135360/dotfiles.git` 的 `main`，远端从 `90a7b5b` 前进到 `dc91f96`。本轮没有同步 live `~/.config`，没有重载桌面运行态。
- 验证：`git fetch origin && git rev-list --left-right --count HEAD...origin/main` 在提交前返回 `0 0`；提交前 `tests/run.sh full` 全量通过，`git diff --check` 通过；`git push origin main` 返回 `90a7b5b..dc91f96  main -> main`。
- 后续：本条发布记录将作为 trace-only 提交再推送一次；追加 trace 本身不递归追加第二条发布记录。

## 2026-06-04

- 目的：按用户要求继续做 Awesome 桌面体验优化，先落地只读分析中收益最高的多屏 tasklist 改进，避免非聚焦屏当前标签有窗口时顶栏中间区没有可读窗口标题。
- 已做：先确认 repo/live Awesome 配置只读比较仅发现 live 额外存在可选 `collision` 目录；随后在 `tests/awesome_ui_architecture_test.sh` 中新增失败用例，锁定 tasklist source/filter 必须优先显示当前屏幕的全局焦点窗口，并在焦点位于其它屏幕时回退到该屏 focus history 最近当前标签窗口或第一个可见普通窗口。实现上将 `.config/linux/awesome/ui/tasklist.lua` 从全局 `client.focus` source 改为 screen-local source/filter，排除 minimized/hidden/skip_taskbar/dock/desktop/splash 等非普通可见窗口；同步更新 `.config/linux/awesome/README.md`、`tests/awesome_docs_theme_test.sh` 与 `memory/awesome.md`，把 tasklist 语义改为“每屏只显示一个当前标签窗口”。
- 验证：新增 `tests/awesome_ui_architecture_test.sh` 在旧实现上先失败；实现后 `sh -n tests/awesome_ui_architecture_test.sh tests/awesome_docs_theme_test.sh tests/awesome_layout_test.sh`、`lua -e 'assert(loadfile(".config/linux/awesome/ui/tasklist.lua")); assert(loadfile(".config/linux/awesome/ui/wibar.lua"))'`、`tests/awesome_docs_theme_test.sh`、`tests/awesome_layout_test.sh`、`tests/awesome_ui_architecture_test.sh`、完整 `for t in tests/awesome_*_test.sh; do "$t"; done`、`awesome -k -c "$PWD/.config/linux/awesome/rc.lua"` 与相关文件 `git diff --check` 均通过。
- 后续：本轮只修改仓库文件，没有同步 live `~/.config/awesome`，没有重载 Awesome，也没有提交推送；若要当前桌面立即看到多屏 tasklist 改进，需要另行同步 live Awesome 配置并重载。下一段可继续评估布局控制（master 数/列数或更多布局）与窗口菜单键盘入口。

## 2026-06-04

- 目的：按用户要求将当前 Awesome 多屏 tasklist 体验优化发布到远程 GitHub。
