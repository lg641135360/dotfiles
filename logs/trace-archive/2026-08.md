...
- 回滚信息：未提交；仓库侧 `git checkout -- .config/scripts/wayland-autostart .config/linux/niri/README.md tests/wayland_scripts_test.sh memory/niri.md` 撤回本轮（会连同其它未提交改动一并撤回，慎用）；live 恢复 `cp ~/.config/scripts/wayland-autostart.backup.<时间戳> ~/.config/scripts/wayland-autostart`。
